...
def test_all_six_third_growth_forms_are_reachable():
    cases = {
        'route_a1': {'사유': 12, '탐구': 4, '감정': 1, '감각': 1},
        'route_a2': {'사유': 4, '탐구': 12, '감정': 1, '감각': 1},
        'route_b1': {'사유': 1, '탐구': 1, '감정': 12, '감각': 4},
        'route_b2': {'사유': 1, '탐구': 1, '감정': 4, '감각': 12},
        # C1: mixed response modes dominate the complexity.
        'route_c1': {'사유': 8, '탐구': 7, '감정': 7, '감각': 2, '상상': 1},
        # C2: multiple world/topic axes dominate the complexity.
        'route_c2': {'사유': 2, '감정': 2, '상상': 7, '사회': 6, '자연': 5},
    }
    reached = set()
    for expected, stats in cases.items():
        parent = expected[:7]  # route_a / route_b / route_c
        d = resolve_growth_route(stats, 20, current_form=parent)
        reached.add(d.form_id)
        assert d.form_id == expected
    assert reached == set(cases)


def test_every_third_form_has_reachable_alpha_and_beta_final():
