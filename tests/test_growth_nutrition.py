...
def test_near_null_response_does_not_feed_growth():
    ...
    n=project_growth_nutrition('책갈피 위치를 다시 확인했다.',a)
    assert n.response=={}
...
def test_secondary_response_accumulates_more_slowly():
    a=_analysis(response=['감정','사유'],scores={'감정':.91,'사유':.885},rn=.84)
    n=project_growth_nutrition('마음이 아팠고 그 선택의 책임도 생각했다.',a)
    assert n.response=={'감정':1.0,'사유':.55}


def test_weak_semantic_world_guess_is_ignored():
    a=_analysis(world=['상상'],scores={'상상':.852},wn=.84,evidence={'상상':0})
...
def test_world_is_capped_to_two_and_secondary_is_downweighted():
    ...
    n=project_growth_nutrition('재난 뒤 주민들이 밀려나는 장면이 불안했다.',a)
