...
BOOKKEEPING_CUE = re.compile(
    ...
    r'마지막\s*읽은\s*위치|읽던\s*페이지|읽은\s*위치|시리즈\s*순서|권수|도서관\s*검색|상호대차|'
    r'앱\s*새로고침|새로고침|좌석|사물함|출판연도|발행일|판권면|판본|상권|하권|책등|스티커|'
...
NAMING_OR_LABEL_CUE = re.compile(
    ...
    r'(?:이라는|라는)\s*(?:브랜드|과목명)|제품명|활동명|카드\s*이름|분류(?:표시|기호)|ISBN|출판연도|판본|여행용)'
)
